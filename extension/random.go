...
func randomGeneratorConstructor(args object.CallArgs) (object.Object, error) {
	...
	if _, ok := seedObj.(object.Unit); !ok {
		...
		if !ok {
			return nil, object.NewTypeError("Generator() argument 'seed' must be unit or int, got %s", seedObj.TypeName())
		}
		seed = int64(value)
	}
...
func (g *RandomGenerator) randomInt(args object.CallArgs) (object.Object, error) {
	...
	if !ok {
		return nil, object.NewTypeError("int() argument 'max' must be unit or int, got %s", maxObj.TypeName())
	}
	...
	if hasMin {
		...
		if !ok {
			return nil, object.NewTypeError("int() argument 'min' must be an int, got %s", minObj.TypeName())
		}
	}
	if min >= max {
...
func (g *RandomGenerator) randomChoice(args object.CallArgs) (object.Object, error) {
	...
	if !ok {
		return nil, object.NewTypeError("choice() argument 'list' must be a list, got %s", listObj.TypeName())
	}
...
func (g *RandomGenerator) randomSample(args object.CallArgs) (object.Object, error) {
	...
	list, ok := listObj.(*object.List)
	if !ok {
		return nil, object.NewTypeError("sample() argument 'list' must be a list, got %s", listObj.TypeName())
	}
...
func (g *RandomGenerator) TypeName() string { return "Generator" }
